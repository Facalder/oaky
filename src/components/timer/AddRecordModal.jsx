...
export default function AddRecordModal({ isOpen, onClose, selectedDate }) {
  ...
  const [selectedTaskId, setSelectedTaskId] = useState(tasks.length > 0 ? tasks[0].id : null);
    ...
    setRecords(prev => {
      ...
      return {
        ...
        [dateStr]: {
          ...
          tasks: {
            ...todayRecord.tasks,
            [activeTask.id]: (todayRecord.tasks[activeTask.id] || 0) + durationSeconds
          },
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md p-0 overflow-hidden bg-[#1c1c1e] text-white rounded-3xl border-0 shadow-2xl">
        <div className="bg-white text-gray-900 rounded-t-3xl p-8 pb-10">
          ...
          <div className="flex items-center justify-between bg-white border border-gray-200 rounded-xl p-4 mb-8 cursor-pointer hover:bg-gray-50 transition-colors">
            <div className="flex items-center gap-3">
              <div className={`w-4 h-4 rounded-sm ${activeTask?.color || "bg-gray-200"}`}></div>
              <span className="font-medium">{activeTask?.title || "Choose a task"}</span>
            </div>
        ...
        <div className="grid grid-cols-2 bg-[#1c1c1e]">
          <button onClick={onClose} className="py-5 font-medium text-gray-400 hover:text-white transition-colors border-r border-gray-700/50">
            Cancel
          </button>
          <button onClick={handleSave} className="py-5 font-medium text-white hover:bg-[#2c2c2e] transition-colors rounded-br-3xl">
            Add record
          </button>
        </div>
      </DialogContent>

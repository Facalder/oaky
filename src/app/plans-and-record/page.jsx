...
import { ChevronLeft, ListTodo, Eraser, Check, Plus, MoreVertical, AlignRight } from "lucide-react";
...
export default function PlansPage() {
  ...
  const hoursGrid = Array.from({ length: 12 }, (_, i) => {
    const hour = i + 5;
    const label = hour > 12 ? (hour - 12).toString().padStart(2, "0") : hour.toString().padStart(2, "0");
  ...
  const getBlockStyle = (colorClass) => {
    ...
    return colorMap[colorClass] || { bg: "#f3f4f6", border: "#e5e7eb", solid: "#9ca3af" };
  };
    ...
    Object.keys(groupedData).map((category) => (
      <div key={category} className="mb-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-semibold text-[17px] text-gray-900">{category}</h3>
          <span className="text-[#5b45c2] text-xs font-mono font-medium">0h 00m</span>
        </div>
            ...
            return (
              <div key={task.id} className="flex items-start justify-between group">
                <div className="flex gap-4 items-start">
                  <div className={`w-[4px] h-[26px] mt-1 ${task.color} rounded-full ${isCompleted ? "opacity-40" : ""}`}></div>
                  <div>
                    <p className={`text-[16px] font-medium mb-1.5 transition-all ${isCompleted ? "line-through text-gray-400" : "text-gray-600"}`}>
                      {task.title}
                    </p>
                    <p className={`text-[12px] ${isCompleted ? "text-gray-300" : "text-gray-400"}`}>
                      {task.startAt || "20:00"} - {task.endAt || "22:00"}
                    </p>
                    <p className={`text-[12px] mt-0.5 ${isCompleted ? "text-gray-300" : "text-gray-400"}`}>
                ...
                <div className="flex flex-col items-end gap-2 mt-1">
                  <div className="flex items-center gap-3 text-xs text-gray-500 font-mono">
                    <span className="uppercase font-sans font-medium text-[10px] tracking-wider">Total :</span>
                    <span className="text-gray-900 font-medium">{formatTotalTime(taskRecordSecs)}</span>
  ...
  return (
    <div className="p-8 pt-0 grid grid-cols-1 lg:grid-cols-[500px_1fr] gap-8 h-full">
      ...
      <div className="bg-white rounded-[32px] p-8 shadow-sm border border-gray-50 flex flex-col h-full min-h-[500px]">
        ...
        <div className="flex items-center justify-between mb-8 relative">
          <div className="flex items-center gap-4">
            <button onClick={handlePrevDay} className="p-1 hover:bg-gray-100 rounded-md transition-colors">
              <ChevronLeft size={24} strokeWidth={2} className="text-gray-800" />
            </button>
      ...
      <div className="bg-white rounded-[32px] p-8 shadow-sm border border-gray-50 flex flex-col h-full overflow-hidden relative">
        ...
        <div className="flex w-full pt-4 pb-8 border-b border-gray-100 relative">
          <div className="flex-1 flex flex-col items-center justify-center">
            <span className="text-[11px] text-gray-400 font-medium tracking-widest mb-1 uppercase">Plan</span>
            <span className="text-3xl font-mono text-gray-900">{planHours}h {planMins.toString().padStart(2, "0")}m</span>
          </div>
          <div className="w-[1px] h-full bg-gray-100 absolute left-1/2 top-0 bottom-0"></div>
          <div className="flex-1 flex flex-col items-center justify-center">
            <span className="text-[11px] text-gray-400 font-medium tracking-widest mb-1 uppercase">Record</span>
            <span className="text-3xl font-mono text-gray-900">{recordHours}h {recordMins.toString().padStart(2, "0")}m</span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar relative mt-6 pr-4">
          <div className="relative w-full h-[720px]">
            ...
            {hoursGrid.map((grid, index) => (
              <div key={index} className="absolute w-full flex items-start" style={{ top: `${index * 60}px` }}>
                <div className="w-12 flex items-center justify-between pr-4 relative -top-3">
                  {grid.isPM && grid.hour === 12 && <span className="text-[10px] text-gray-400 font-medium absolute -left-6">PM</span>}
                  {!grid.isPM && grid.hour === 12 && <span className="text-[10px] text-gray-400 font-medium absolute -left-6">AM</span>}
                  <span className="text-xs text-gray-400 font-mono w-full text-right">{grid.label}</span>
                </div>
            ...
            <div className="absolute left-[70px] w-[calc(50%-40px)] top-0 bottom-0 pointer-events-none">
              {activeTasks.map((task) => {
                ...
                return (
                  <div key={`plan-${task.id}`} className="absolute rounded-[10px] p-3 overflow-hidden flex items-start z-10"
                    style={{ top: `${top}px`, height: `${height}px`, width: "100%", backgroundColor: colors.bg, borderColor: colors.border, borderWidth: "1px" }}>
                    <span className="text-[13px] font-medium text-gray-600 leading-tight">{task.title}</span>
                  </div>
                );
              })}
            </div>
            ...
            <div className="absolute left-[calc(50%+24px)] w-[calc(50%-40px)] top-0 bottom-0 pointer-events-none">
              {dailyRecord.sessions && dailyRecord.sessions.map((session) => {
                ...
                return (
                  <div key={`rec-${session.id}`} className="absolute rounded-[10px] p-3 overflow-hidden flex items-start shadow-sm z-10"
                    style={{ top: `${top}px`, height: `${height}px`, width: "100%", backgroundColor: colors.solid, border: "none" }}>
                    <span className="text-[13px] font-medium text-gray-900 leading-tight mix-blend-color-burn">{session.title}</span>
                  </div>
                );
              })}
            </div>
      ...
      <AddRecordModal isOpen={isRecordModalOpen} onClose={() => setIsRecordModalOpen(false)} selectedDate={selectedDate} />
    </div>
  );
}
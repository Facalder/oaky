    ...
    <aside className='w-64 h-screen bg-[#f8f9fa] flex flex-col justify-between border-r border-gray-100 p-4 shrink-0'>
      <div>
        ...
        <nav className='flex flex-col gap-1'>
          <Link
            href='/'
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl font-medium text-sm transition-colors ${isActive('/') ? 'bg-[#efedf8] text-[#5b45c2]' : 'text-gray-600 hover:bg-gray-100'}`}
          >
            <Clock size={18} /> Timer
          </Link>
          <Link
            href='/plans-and-record'
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl font-medium text-sm transition-colors ${isActive('/plans') ? 'bg-[#efedf8] text-[#5b45c2]' : 'text-gray-600 hover:bg-gray-100'}`}
          >
            <Calendar size={18} /> Plans and Records
          </Link>
          <Link
            href='/statistic'
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl font-medium text-sm transition-colors ${isActive('/statistic') ? 'bg-[#efedf8] text-[#5b45c2]' : 'text-gray-600 hover:bg-gray-100'}`}
          >
            <BarChart2 size={18} /> Statistics
          </Link>
          <Link
            href='/diary'
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl font-medium text-sm transition-colors ${isActive('/diary') ? 'bg-[#efedf8] text-[#5b45c2]' : 'text-gray-600 hover:bg-gray-100'}`}
          >
